...
void returnData(uint8_t address){
    switch (address) {
        ...
        case 8:
            ...
            slUART_LogBinaryNl(bitRead(dataIn[1],0)+(bitRead(dataIn[1],1)<<1)+(bitRead(dataIn[1],2)<<2)+(bitRead(dataIn[1],3)<<3));
            slUART_WriteString("PLOS_CNT = ");
            slUART_LogBinaryNl(bitRead(dataIn[1],4)+(bitRead(dataIn[1],5)<<1)+(bitRead(dataIn[1],6)<<2)+(bitRead(dataIn[1],7)<<3));
            break;//8
        case 9:
...
void slNRF_SetRXPayload(uint8_t pipe, uint8_t bytes) {
    ...
    CSN_LOW();
    dataIn[0] = slSPI_TransferByte(address);
    CSN_HIGH();
    CSN_LOW();
    dataIn[1] = slSPI_TransferByte(bytes);
    CSN_HIGH();
...
void slNRF_GetAddress(uint8_t address, uint8_t log) {
    ...
    CSN_HIGH();
    CSN_LOW();
    dataIn[1] = slSPI_TransferByte(0x00);
    CSN_HIGH();
    slUART_WriteString("dataIn[0]: ");
    slUART_LogBinaryNl(dataIn[0]);
    slUART_WriteString("dataIn[1]: ");
...
void slNRF_BitWrite(uint8_t address, uint8_t bit_add, uint8_t val){//   start bit write   start bit write   start bit write
    ...
    slUART_LogBinaryNl(dataIn[1]);
    CSN_LOW();
    dataIn[0] = slSPI_TransferByte(32+address);//a write to a register adds 32
    CSN_HIGH();
    CSN_LOW();
    dataIn[1] = slSPI_TransferByte(dataIn[1]);//write the modified register
    CSN_HIGH();
    slUART_WriteString("dataIn[0]: ");
    slUART_LogBinaryNl(dataIn[0]);
    slUART_WriteString("dataIn[1]: ");
